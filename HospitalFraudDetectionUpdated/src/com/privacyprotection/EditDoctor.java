...
public class EditDoctor extends HttpServlet 
{
	...
	private void loginRequest(HttpServletRequest request,
		...
		try{
			...
			int rs = st.executeUpdate("update doctor_register set password ='"+password+"' where username= '"+username+"'");
			System.out.println("password is");
			if(rs>0){
	            ...
	            response.sendRedirect("doctorhome.jsp?update=success");
				
				
			}else{
				System.out.println("User profile Updation failed");
					response.sendRedirect("editDoctor.jsp?update=fail");
				}
